...
export type HubBindings =
	| HubListenerByFullSelector
	| HubListenerByHubNameBySelector
	| (HubListenerByFullSelector | HubListenerByHubNameBySelector)[];
...
class HubImpl implements Hub {
	...
	sub(
		topics: string,
		labels_or_handler: string | HubListener,
		handler_or_opts?: HubListener | HubOptions,
		opts?: HubOptions,
	) {
		//// Build the arguments
