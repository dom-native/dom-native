...
const _pushers: [string, PusherFn][] = [
	[
		...
		function (this: HTMLInputElement, value: any) {
	...
	[
		"select",
		function (this: HTMLSelectElement, value: any) {
			this.value = value;
		},
	],
	...
	[
		"textarea",
		function (this: HTMLTextAreaElement, value: any) {
			this.value = value;
		},
	],
...
const _pullers: [string, PullerFn][] = [
	[
		"input[type='checkbox'], input[type='radio']",
		function (this: HTMLInputElement, existingValue: any) {
...
function getPropPath(dxEl: HTMLElement) {
	let path = null;
	let i = 0,
		classes = dxEl.classList,
		l = dxEl.classList.length,
		name;
	for (; i < l; i++) {
		name = classes[i];
