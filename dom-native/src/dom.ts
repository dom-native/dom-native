...
function process_arg_el_selectors(
	el_or_selectors: MaybeEl | string | string[],
	maybe_selectors: string[],
): [MaybeEl, string[]] {
 ...
 */
export function first<A extends TagName | String>(
	selector: A,
): A extends TagName ? HTMLElementTagNameMap[A] | null : HTMLElement | null;
/**
 ...
 */
export function getFirst<A extends TagName | String>(
	selector: A,
): A extends TagName ? HTMLElementTagNameMap[A] : HTMLElement;
/**
...
export function getFirst(
	el_or_selectors: El | string | string[],
	...maybe_selectors: string[]
 ...
 */
export function cherryChild<A extends TagName | String>(
	el: El,
	tagName: A,
): A extends TagName ? HTMLElementTagNameMap[A] : HTMLElement;
export function cherryChild<A extends (TagName | String)[]>(
...
export function cherryChild(
	el: Document | HTMLElement | DocumentFragment,
	...tagNames: string[]
): HTMLElement | HTMLElement[] {
	if (el == null) {
		throw new Error(`dom-native - cherryChild - requires el to not be null`);
	}
	...
	if (result!.length < tagNamesLength) {
		throw new Error("dom-native - cherryChild - node has less match children than requested");
	}

	return result!;
}
// #endregion --- cherryChild
...
function _execQuerySelector(
	all: false,
	elOrSelector?: Document | HTMLElement | DocumentFragment | null | string,
	selector?: string,
): Element | null;
function _execQuerySelector(
	all: true,
...
// #region    --- append
export function append<T extends HTMLElement | HTMLElement[] | DocumentFragment | string>(
	this: any,
	refEl: HTMLElement | DocumentFragment,
	newEl: T,
	position?: AppendPosition,
): T extends HTMLElement ? HTMLElement : HTMLElement[];

export function append(
	this: any,
	refEl: HTMLElement | DocumentFragment,
	newEl: HTMLElement | HTMLElement[] | DocumentFragment | string,
	position?: AppendPosition,
): HTMLElement | HTMLElement[] {
	...
	if (position === "last" || position === "first" || position === "empty") {
		...
		if (!parentEl) {
			throw new Error(
				"dom-native ERROR - The referenceElement " + refEl + " does not have a parentNode. Cannot insert " + position,
			);
		}
	}
